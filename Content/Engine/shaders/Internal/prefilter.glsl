GLSLShader
{
    ...
    SubShader
    {
        ...
        { 
            ...
            uniform float _CubeMapResolution;
            void main() 
            {
                ...
                for (uint i = 0u; i < SAMPLE_COUNT; ++i)
                {
                    ...
                    if (NdotL > 0.0)
                    {
                        ...
                        float pdf = D * NdotH / (4.0 * HdotV + 0.0001) + 0.0001;

                        float resolution = max(_CubeMapResolution, 1.0);
                        float saTexel = 4.0 * PI / (6.0 * resolution * resolution);
                        float saSample = 1.0 / (float(SAMPLE_COUNT) * pdf + 0.0001);
                        float mipLevel = _Roughness == 0.0 ? 0.0 : 0.5 * log2(saSample / saTexel);

                        prefilteredColor += textureLod(_cubeMap, L, mipLevel).rgb * NdotL;
                        totalWeight += NdotL;
                    }
                }
