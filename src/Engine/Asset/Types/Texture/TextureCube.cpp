...
bool TextureCube::Create(const TextureDesc& desc, const void* pixelData)
{
    ...
    {
        LogA(LogLevel::ERROR, "TextureCube::Create: invalid size {}x{}", desc.width, desc.height);
        return false;
    }
    ...
    {
        LogA(LogLevel::ERROR, "TextureCube::Create: failed to resolve texture layout (internal=0x{:X})",
             desc.internalFormat);
        return false;
    }
